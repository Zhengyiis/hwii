...
class RandomFgsmAdvTrainer:
    ...
    def _train_batch(self, x, y):
        # reference: https://github.com/locuslab/fast_adversarial/blob/master/CIFAR10/train_fgsm.py
        self.model.eval()

        x_adv = x + torch.randn_like(x).uniform_(-self.adv_config.epsilon, self.adv_config.epsilon)
        x_adv = torch.clamp(x_adv, min=0, max=1).detach()
        x_adv.requires_grad_(True)

        loss_make_adv = F.cross_entropy(self.model(x_adv), y)
        grad = torch.autograd.grad(loss_make_adv, [x_adv])[0]
        x_adv = x_adv.detach() + self.alpha * self.adv_config.epsilon * torch.sign(grad.detach())
