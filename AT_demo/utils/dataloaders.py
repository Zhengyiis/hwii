...
from utils.datasets import CIFAR10WithID, CIFAR100WithID, SVHNWithID, GTSRBWithID, STL10WithID, TinyImageNet, \
    TinyImageNetWithID
...
def get_SVHN_loader(train_id=False, test_id=False):
    bs_train = 128
    bs_eval = 256
    ...
    if train_id:
        trainset = SVHNWithID(dir_dataset, train=True, transform=transform_train)
    else:
        trainset = datasets.SVHN(dir_dataset.joinpath('SVHN'), split='train', download=False, transform=transform_train)

    targets = np.asarray(trainset.targets, dtype=np.int32)

    if test_id:
        testset = SVHNWithID(dir_dataset, train=False, transform=transform_test)
    else:
        testset = datasets.SVHN(dir_dataset.joinpath('SVHN'), split='test', download=False, transform=transform_test)
...
def get_GTSRB_loader(train_id=False, test_id=False):
    bs_train = 128
    bs_eval = 256
    ...
    if train_id:
        trainset = GTSRBWithID(dir_dataset, train=True, transform=transform_train)
    else:
        trainset = datasets.GTSRB(dir_dataset, split='train', download=False, transform=transform_train)

    targets = np.asarray(trainset.get_targets())

    if test_id:
        testset = GTSRBWithID(dir_dataset, train=False, transform=transform_test)
    else:
        testset = datasets.GTSRB(dir_dataset, split='test', download=True, transform=transform_test)
...
def get_STL10_loader(train_id=False, test_id=False):
    bs_train = 128
    bs_eval = 256
    ...
    if train_id:
        trainset = STL10WithID(dir_dataset, train=True, transform=transform_train)
    else:
        trainset = datasets.STL10(dir_dataset, split='train', download=False, transform=transform_train)

    targets = np.asarray(trainset.targets, dtype=np.int32)

    if test_id:
        testset = STL10WithID(dir_dataset, train=False, transform=transform_test)
    else:
        testset = datasets.STL10(dir_dataset, split='test', download=False, transform=transform_test)
